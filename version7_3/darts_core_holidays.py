...
#   当前实验版本提交文件输出路径。
# CORE_HOLIDAYS:
#   本次节假日筛选后仅保留的高影响核心节日集合。
# 这个版本沿用的主要因子:
#   与 version6.3 保持一致，包括 static covariates、transactions、oil、
#   onpromotion、日期因子以及 7 / 63 / 365 / 730 多组 lags。
# 这个版本唯一改动:
#   节假日因子从原来的 7 个候选，缩减为 4 个核心节日，用于验证噪声节日是否拖累分数。
LGBM_WEIGHT = 0.40
XGB_WEIGHT = 0.60
OUTPUT_PATH = "version7_3/submission_darts_core_holidays.csv"
CORE_HOLIDAYS = [
    "nat_navidad",
    "nat_dia trabajo",
    "nat_futbol",
...
def main():
    ...
    # LightGBM 分支：保留与 version6.3 一致的 lags 结构。
    ...
    # XGBoost 分支：保持同一组 lags，仅测试节假日因子筛选的影响。
    xgb_base = {
        **base_config,
        "n_estimators": 100,
        "learning_rate": 0.1,
    ...
    print(f"Single-factor change: keep only core holidays {selected_holidays}")
