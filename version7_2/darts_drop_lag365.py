...
#   onpromotion、日期因子、节假日因子以及 7 / 63 / 730 多组 lags。
# 这个版本唯一改动:
#   移除 365 年周期滞后分支，用于单独验证一年周期信息是否必要。
LGBM_WEIGHT = 0.40
XGB_WEIGHT = 0.60
OUTPUT_PATH = "version7_2/submission_darts_drop_lag365.csv"
...
def main():
    ...
    # LightGBM 分支：本实验只去掉 lag=365，其余配置保持不变。
    ...
    # XGBoost 分支：同步去掉 lag=365，保持对照公平。
    xgb_base = {
        **base_config,
        "n_estimators": 100,
        "learning_rate": 0.1,
    ...
    print("Single-factor change: removed lag=365 branch only")
