...
#   最终融合阶段的模型权重，本版本固定为 0.35 / 0.65。
# OUTPUT_PATH:
#   当前实验版本提交文件输出路径。
# 这个版本沿用的主要因子:
#   与 version6 主线完全一致，包括 static covariates、transactions、oil、
#   onpromotion、日期因子、节假日因子以及 7 / 63 / 365 / 730 多组 lags。
# 这个版本唯一改动:
#   只测试“进一步增加 XGBoost 权重”是否还能继续降分。
LGBM_WEIGHT = 0.35
XGB_WEIGHT = 0.65
OUTPUT_PATH = "version6_4/submission_darts_weighted_ensemble.csv"
